...
以下の操作時は対応するサブエージェントを使用する。

| 操作 | サブエージェント | 説明 |
|------|-----------------|------|
| マージ後処理 | `git-ops` | fetch --prune、worktree削除、rebase |
| PR作成 | `git-ops` | ブランチ作成、コミット、PR作成 |
| ドキュメント作成 | `doc-writer` | README、API仕様書 |
| コミット前レビュー | `code-reviewer` | セキュリティ・品質チェック |
