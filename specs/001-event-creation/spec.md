# Feature Specification: フェーズ 1：イベント投稿機能

**Feature Branch**: `001-event-creation`
**Created**: 2025-11-11
**Status**: Draft
**Input**: User description: "フェーズ 1：イベント投稿機能 - 匿名投稿によるイベント作成とタイムライン表示"
...
- Q: 認証方式 → A: 開発初期段階ではSupabase Anonymous Sign-inを使用し、表示名のみで簡易登録・ログイン可能にする（メールアドレス不要）
- Q: セッション維持期間 → A: Supabaseのデフォルト設定に従い、24時間以上維持する
- Q: ログアウト後のデータ引き継ぎ → A: 匿名認証のため、ログアウト後の再登録時は新しいユーザーとして扱い、以前のデータは引き継がない
...
5. **Given** タイムラインに投稿が 0 件, **When** ホーム画面を開く, **Then** 「まだイベントが投稿されていません。右下の＋ボタンから最初のイベントを投稿しましょう!」というガイドメッセージが表示される
...
### User Story 4 - 簡易認証機能（匿名ログイン） (Priority: P2)

ユーザーが簡易的な認証機能を使ってログインし、イベント投稿・閲覧機能を利用できる。開発初期段階では匿名ログイン（メールアドレス不要）でユーザー識別を行う。

**Why this priority**: US1（イベント投稿）とUS2（タイムライン閲覧）の前提条件。認証なしでは投稿者の識別やつながりリストベースの配信が実現できない。ただし、初期MVPでは本格的な認証（メールアドレス認証、OAuth等）は不要で、簡易的なユーザー識別のみで十分。

**Independent Test**: 初回訪問時にユーザー作成画面が表示され、表示名を入力後に自動的にログイン状態となり、イベント投稿・タイムライン閲覧が可能になることを確認することで独立してテスト可能。

**Acceptance Scenarios**:

1. **Given** 初回訪問のユーザーがアプリを開く, **When** ホーム画面にアクセスする, **Then** 「ゆるぼへようこそ」という簡易登録画面が表示され、表示名入力フィールドと「はじめる」ボタンが表示される
2. **Given** 簡易登録画面が表示されている, **When** 表示名「テストユーザー」を入力して「はじめる」ボタンを押す, **Then** 自動的にユーザーが作成され、ログイン状態となり、タイムライン画面に遷移する
3. **Given** ログイン済みのユーザーがアプリを開く, **When** ホーム画面にアクセスする, **Then** 登録画面をスキップし、直接タイムライン画面が表示される（セッション維持）
4. **Given** ログイン済みのユーザーがアプリを使用している, **When** ブラウザを閉じて再度開く, **Then** ログイン状態が維持され、再度登録画面は表示されない
5. **Given** ログイン済みのユーザー, **When** 設定画面から「ログアウト」ボタンを押す, **Then** ログアウトし、次回アクセス時に登録画面が再表示される
...
- **表示名が空の場合**: 登録時にバリデーションエラー「表示名を入力してください」を表示
- **表示名が 50 文字を超える場合**: 登録時にバリデーションエラー「表示名は 50 文字以内で入力してください」を表示
- **セッション有効期限切れ**: 長期間アクセスがない場合、次回アクセス時に自動的に再登録画面が表示される
- **ログアウト後の再アクセス**: ログアウトしたユーザーは次回アクセス時に新しい匿名ユーザーとして登録され、以前のデータは引き継がれない
...
- **FR-021**: システムは初回訪問ユーザーに対して簡易登録画面を表示しなければならない
- **FR-022**: システムはユーザーが表示名のみで登録・ログインできるようにしなければならない（メールアドレス不要）
- **FR-023**: システムはSupabase Anonymous Sign-inを使用してユーザーを作成し、セッションを維持しなければならない
- **FR-024**: システムはログイン状態を維持し、再訪問時に自動ログインしなければならない
- **FR-025**: システムはユーザーがログアウトできる機能を提供しなければならない
- **FR-026**: システムは表示名のバリデーションを実施しなければならない（1〜50文字、必須）
...
- **SC-008**: ユーザーは初回訪問時に 10 秒以内に登録を完了できる（表示名入力のみ）
- **SC-009**: ログイン状態は 24 時間以上維持され、ユーザーは再ログイン不要でアプリを利用できる
- **SC-010**: 95%のユーザーが表示名入力で登録を一度で成功できる（バリデーションエラーなし）
